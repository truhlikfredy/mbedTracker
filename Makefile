...
C_FLAGS += -DMBED_BUILD_TIMESTAMP=1485997395.52
...
CXX_FLAGS += -DMBED_BUILD_TIMESTAMP=1485997395.52
...
all: flash $(PROJECT).bin $(PROJECT).hex size
...
flash: $(PROJECT).bin
	+@echo "Copying binary..." 
	rm -f /media/fredy/MBED/$(PROJECT).bin 2>/dev/null
	cp $(PROJECT).bin /media/fredy/MBED/$(PROJECT).bin
	sync
	sleep 3
	+@echo "Reseting device"
	stty -F /dev/ttyACM0 9600
	perl -e 'use POSIX; tcsendbreak(3, 0)' 3>/dev/ttyACM0
	+@echo "Connect to serial with minicom -D /dev/ttyACM0"
	+@echo "Or make script nxpSerial.sh"
